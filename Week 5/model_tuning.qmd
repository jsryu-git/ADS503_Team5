...
#eliminate re-sampling for the final model
ctrl_final <- trainControl(
  method          = "none",          #scrapping the cross validation, suggested by gemini
  classProbs      = TRUE,
  summaryFunction = twoClassSummary,
  allowParallel   = FALSE,
  trim            = TRUE,            # trimming unnecessary data, suggested by gemini
  returnData      = TRUE          
)
...
```{r export-workspace}
# Set working directory to ensure it saves to your project data folder
setwd(r"(C:\Users\User\ADS503_Team5\Raw Data)")
