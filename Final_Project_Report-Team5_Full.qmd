...
df_full |>
  ...
  scale_fill_manual(values = dm_palette) +
  labs(title = "Distribution of continuous predictors by class",
...
# Fit preprocessing only on training data, then apply to both train and test
pp <- preProcess(train_x, method = c("zv", "center", "scale"))

x_train_df <- as.data.frame(predict(pp, train_x))
x_test_df <- as.data.frame(predict(pp, test_x))
...
set.seed(503)
cv_index <- createFolds(train_y, k = 5, returnTrain = TRUE)
...
# Re-using the cross-validation controller to find the optimal hyperparameters
set.seed(503)
mod_gbm <- train(
  x         = x_train_df, 
  y         = train_y,
...
# Explicitly shut down the cluster to release resources
stopCluster(cl)
registerDoSEQ()
```
